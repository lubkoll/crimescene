...
def _run_git_cmd(root, git_arguments):
    return subprocess.Popen(git_arguments, stdout=subprocess.PIPE, cwd=root).communicate()[0].decode("utf-8") 
...
def _git_cmd_for(rev_start, rev_end):
    ...
    return ['git', 'log', rev_range, "--format='%h %ad %s'", '--date=raw']
#    return ['git', 'log', rev_range, '--oneline']

def read_revs(root, rev_start, rev_end):
    """ Returns a list of all commits in the given range.
    """
    return _read_revisions_matching(root, git_arguments=_git_cmd_for(rev_start, rev_end))

def read_revs_for(root: str, file_name, rev_start, rev_end):
    return _read_revisions_matching(root, git_arguments=_git_cmd_for(rev_start, rev_end) + [file_name])
...
def read_commit_messages(root, start: str, end: str):
    return _run_git_cmd(root, ['git', 'log', "--pretty=format:'%s'", f'--after={start}', f'--before={end}']).replace('\n', ' ').replace("'","")
